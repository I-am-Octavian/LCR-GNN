...
def forward(data, model, device, writer, dataloader, sumfact_pool_dataset, referissue_pool_dataset, label_dict, yf_path, epoch, temp, bm25_hard_neg_dict, hard_neg, hard_neg_num, train_flag, embedding_saving, optimizer=None):
    if train_flag:
        ...
        for batched_graph, labels in tqdm(dataloader):
            ...
            for x in range(len(batched_case_list)):
                ...
                pos_case = random.choice(label_dict[query_name]).split('.')[0].zfill(6)
        ...
        with torch.no_grad():
            ...
            for key, value in label_dict.items():
                ...
                query_index = test_label_list.index(key.split('.')[0].zfill(6))
            ...
            print("MAP: ", map_score)
            print("Retrieval Precision@5", p_score)
    ...
    if embedding_saving:
        ...
        torch.save(case_embedding_matrix, os.getcwd()+'/Graph_generation/'+dataset+'_casegnn_embedding.pt')

        with open(os.getcwd()+'/Graph_generation/'+dataset+'_casegnn_embedding_case_name_list.json' , "w") as fOut:
